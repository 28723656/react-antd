...
class UpdateParentModalClass extends Component{
    ...
    handleOk =(e) =>{
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                // 如果有id,就是修改
                if (values.id) {
                    updateAjax('/plan/dictParent', values)
                        .then(response => {
                            initData();
                    ...
                    // 没有就是插入
                    addAjax('/plan/dictParent', values)
                        .then(response => {
                            initData();
                            handleModalDisappear();
                        })
...
const UpdateParentModal = Form.create({name: 'UpdateParentForm1'})(UpdateParentModalClass);
